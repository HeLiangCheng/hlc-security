...
public class UserDetailServiceImpl implements UserDetailsService,SocialUserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if ("admin".equals(username)) {
            ...
            user = new User("admin", pwdStr, true, true, true, true, AuthorityUtils.commaSeparatedStringToAuthorityList("ROLE_ADMIN,ROLE_USER"));
            logger.info("验证成功");
        }
        //按手机号码查找
        if ("123456".equals(username)) {
            logger.info("开始验证用户手机验证码");
            user = new User("admin", "123456", true, true, true, true, AuthorityUtils.commaSeparatedStringToAuthorityList("ROLE_ADMIN,ROLE_USER"));
            logger.info("验证成功");
        }
        return user;
    }
